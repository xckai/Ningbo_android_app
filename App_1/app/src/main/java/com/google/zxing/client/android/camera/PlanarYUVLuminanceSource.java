...
//      row = new byte[height];
//    }
//    int offset = (y + top) * dataHeight + top;
//    System.arraycopy(yuvData, offset, row, 0, height);
//    return row;
//  }
  @Override
  public byte[] getRow(int y, byte[] row) {
    ...
    int x=y*getWidth()/getHeight();
    int height=getHeight();
    if (row == null || row.length < height) {
     row = new byte[height];
    	}
    int offset=top*dataWidth+x+left;
    for (int i=0;i<height;++i)
    {
    	row[i]=yuvData[offset];
    	offset+=dataWidth;
  ...
  private void reverseHorizontal(int width, int height, byte[] yuvData) {
